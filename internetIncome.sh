...
start_containers() {
  ...
  if [[ $i && $proxy ]]; then
    ...
    if [[ "$USE_SOCKS5_DNS" == "true" && "$proxy" == socks5://* ]]; then
      ...
      check_container_exists tun$UNIQUE_ID$i
      if CONTAINER_ID=$(sudo docker run --name tun$UNIQUE_ID$i $LOGS_PARAM --restart=always --mount type=bind,source=/dev/net/tun,target=/dev/net/tun --cap-add=NET_ADMIN $combined_ports -d ghcr.io/tun2proxy/tun2proxy:v0.8.0 $DNS_OPTION --proxy $proxy --verbosity $TUN_LOG_PARAM); then
        echo -e "${GREEN}Container tun$UNIQUE_ID$i started successfully.${NOCOLOUR}"
      else
        echo -e "${RED}Failed to start container for proxy. Exiting..${NOCOLOUR}"
        exit 1

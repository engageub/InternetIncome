...
start_containers() {
  ...
  if [[ $WIPTER_EMAIL && $WIPTER_PASSWORD ]]; then
    ...
    if [ "$container_pulled" = false ]; then
      sudo docker pull --platform=linux/amd64 ghcr.io/adfly8470/wipter/wipter@sha256:2c0d86dce03e400c5976897be4a7b4183aec722ee294853e920038483d628b1a
    fi
    if CONTAINER_ID=$(sudo docker run -d --platform=linux/amd64 --name wipter$UNIQUE_ID$i --restart=always $LOGS_PARAM $DNS_VOLUME $NETWORK_TUN -e WIPTER_EMAIL=$WIPTER_EMAIL -e WIPTER_PASSWORD=$WIPTER_PASSWORD ghcr.io/adfly8470/wipter/wipter@sha256:2c0d86dce03e400c5976897be4a7b4183aec722ee294853e920038483d628b1a); then
      echo "$CONTAINER_ID" | tee -a $containers_file

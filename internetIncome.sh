...
# Starting Ebesucher Chrome container
  ...
  if [[ $UR_AUTH_TOKEN ]]; then
    ...
    docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $CPU_PARAM $NETWORK_TUN -v "$PWD/$urnetwork_data_folder/data/.urnetwork:/root/.urnetwork" bringyour/community-provider:latest --max-memory=64mib provide)

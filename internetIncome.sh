...
start_containers() {
  ...
  if [[ $i && $proxy && "$START_ONLY" != true ]]; then
    ...
    if [ "$vpn_enabled" = true ];then
      ...
      if [ "$container_pulled" = false ]; then
        sudo docker pull ghcr.io/tun2proxy/tun2proxy:v0.8.0
      fi
      if [[ "$ENABLE_LOGS" != true ]]; then
        TUN_LOG_PARAM="off"
      else
      ...
      docker_parameters=($HOST_NAME $LOGS_PARAM $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM $CUSTOM_NETWORK --sysctl net.ipv6.conf.default.disable_ipv6=0 --device /dev/net/tun --cap-add=NET_ADMIN $combined_ports -d ghcr.io/tun2proxy/tun2proxy:v0.8.0 $dns_option --proxy $proxy --verbosity $TUN_LOG_PARAM)
  ...
  if [ "$AUTO_UPDATE_CONTAINERS" = true ]; then
    if [ "$container_pulled" = false ]; then
      ...
      if ! sudo docker inspect internetincomewatchtower$UNIQUE_ID$i >/dev/null 2>&1; then
        ...
        docker_parameters=($LOGS_PARAM $DNS_VOLUME $MAX_MEMORY_PARAM $MEMORY_RESERVATION_PARAM $MEMORY_SWAP_PARAM $CPU_PARAM -e WATCHTOWER_CLEANUP=true --mount type=bind,source=/var/run/docker.sock,target=/var/run/docker.sock --no-healthcheck containrrr/watchtower:1.7.1 --scope $UNIQUE_ID --interval 86400)

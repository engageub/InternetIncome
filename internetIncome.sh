...
start_containers() {
  ...
  if [[ $i && $proxy ]]; then
    ...
    if [ "$container_pulled" = false ]; then
      sudo docker pull ghcr.io/engageub/tun2proxy:master
    fi
    if CONTAINER_ID=$(sudo docker run --name tun$UNIQUE_ID$i $LOGS_PARAM --restart=always -e LOGLEVEL=$TUN_LOG_PARAM --sysctl net.ipv6.conf.default.disable_ipv6=0 -v '/dev/net/tun:/dev/net/tun' --cap-add=NET_ADMIN $combined_ports -d ghcr.io/engageub/tun2proxy:master --proxy $proxy); then
      echo "$CONTAINER_ID" | tee -a $containers_file
      echo "tun$UNIQUE_ID$i" | tee -a $container_names_file
      sudo docker exec $CONTAINER_ID sh -c 'echo "nameserver 1.1.1.1" > /etc/resolv.conf;echo "nameserver 8.8.8.8" >> /etc/resolv.conf;'
    else
  ...
  if [[ $PACKETSTREAM_CID ]]; then
    ...
    if CONTAINER_ID=$(sudo docker run -d --name packetstream$UNIQUE_ID$i --restart always -e HTTP_PROXY=$proxy -e HTTPS_PROXY=$proxy -e CID=$PACKETSTREAM_CID packetstream/psclient:latest); then
      echo "$CONTAINER_ID" | tee -a $containers_file 
      echo "packetstream$UNIQUE_ID$i" | tee -a $container_names_file
      sleep 10
      sudo docker logs -n 10 "$CONTAINER_ID"
      if sudo docker logs -n 10 "$CONTAINER_ID" 2>&1 | grep -q "PacketStream is running"; then

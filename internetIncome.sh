...
start_containers() {
  ...
  if [[ $i && $proxy ]]; then
    ...
    if CONTAINER_ID=$(sudo docker run --name tun$UNIQUE_ID$i $LOGS_PARAM --restart=always --sysctl net.ipv6.conf.default.disable_ipv6=0 -v '/dev/net/tun:/dev/net/tun' --cap-add=NET_ADMIN $combined_ports -d ghcr.io/tun2proxy/tun2proxy:v0.7.19 $dns_option --proxy $proxy --verbosity $TUN_LOG_PARAM); then
  ...
  if [[ $WIPTER_EMAIL && $WIPTER_PASSWORD ]]; then
    ...
    if CONTAINER_ID=$(sudo docker run -d --name wipter$UNIQUE_ID$i --restart=always $LOGS_PARAM $DNS_VOLUME $NETWORK_TUN -e WIPTER_EMAIL=$WIPTER_EMAIL -e WIPTER_PASSWORD=$WIPTER_PASSWORD ghcr.io/adfly8470/wipter/wipter@sha256:0b85e2380684ccae3ea2ef7431d6b67bb6245f83fb475cccc075ddeda5dcc16d); then
      echo "$CONTAINER_ID" | tee -a $containers_file
  ...
  if [ "$EARNAPP" = true ]; then
    ...
    if [ "$container_pulled" = false ]; then
      ...
      if CONTAINER_ID=$(sudo docker run -d --name dindearnapp$UNIQUE_ID$i $LOGS_PARAM $DNS_VOLUME -v /var/run/docker.sock:/var/run/docker.sock -v $(which docker):/usr/bin/docker -v $PWD:/earnapp docker:18.06.2-dind /bin/sh -c 'apk add --no-cache bash && cd /earnapp && chmod +x /earnapp/restart.sh && while true; do sleep 86400; /earnapp/restart.sh --restartEarnapp; done'); then
        echo "Earnapp restart container started"
        echo "$CONTAINER_ID" | tee -a $containers_file
        echo "dindearnapp$UNIQUE_ID$i" | tee -a $container_names_file
      else
        echo -e "${RED}Failed to start container for earnapp restart. Exiting..${NOCOLOUR}"
        exit 1
      fi
    fi
    ...
    if CONTAINER_ID=$(sudo docker run -d --name earnapp$UNIQUE_ID$i $LOGS_PARAM $DNS_VOLUME --restart=always $NETWORK_TUN -v $PWD/$earnapp_data_folder/data$i:/etc/earnapp -e EARNAPP_UUID=$uuid --no-healthcheck fazalfarhan01/earnapp:lite); then
      echo "$CONTAINER_ID" | tee -a $containers_file
